// backend/mock-services/userService3.ts
import 'dotenv/config';
import express, { Request, Response } from 'express';
import jwt from 'jsonwebtoken';

const app = express();
const PORT = process.env.USER_SERVICE_3_PORT ? parseInt(process.env.USER_SERVICE_3_PORT, 10) : 4004;
...
app.post('/auth/login', (req: Request, res: Response) => {
  ...
  if (username === 'sde_user' && password === 'pass123') {
    ...
    return res.json({ message: 'Login successful (Instance 3)', token, userId: 'USR-101' });
  }
...
app.get('/api/users/profile', (req: Request, res: Response) => {
  res.json({
    service: 'User Microservice Instance 3 (Port 4004)',
    status: 'SUCCESS',
    instanceId: 'USER-REPLICA-3',
    profile: {
...
app.get('/health', (req: Request, res: Response) => {
  res.json({
    service: 'User Microservice Instance 3',
    status: 'UP',
    port: PORT,
...
app.listen(PORT, () => {
  console.log(`👤 User Service Instance 3 running on http://localhost:${PORT}`);
});
